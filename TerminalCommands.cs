...
{
    [HarmonyPatch(typeof(Terminal), nameof(InitTerminal))] [HarmonyPostfix]
    ...
    private static void UpdateCurrentCommand(Terminal __instance)
    {
        ...
        var isCommandValid = CommandsRouter.GetTooltip(out var tooltip);
    ...
    {
        if (commands["ss"] != __instance) return;
